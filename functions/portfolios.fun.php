	...
	if($td) {
		...
		for($i=0; $i<(count($td)-1); $i+=4) {
			...
			for($j=$i; $j<$i+4; $j++){
				...
				if(file_exists("userfiles/portfolio/" . $td[$j]["portfolio_image"]) && !empty($td[$j]["portfolio_image"])) {
					$image = $td[$j]["portfolio_image"];
				} else {
					$image = "default.jpg";
				}
				...
				$content .= "\t\t\t<li><a rel=\"" . $config["path"] . "userfiles/portfolio/" . $image . "\" class=\"image\" title=\"" . $td[$j]["portfolio_title"] . "\"><img   src=\"" . $config["path"] . "userfiles/portfolio/" . $thumbnail . "\" title=\"" . $td[$j]["portfolio_title"] . "\" alt=\"" . $td[$j]["portfolio_brief"] . "\" /></a></li>\n";
...
function GetPortfolioDetails() {
	...
	if($td) {
		...
		$content .= "\t<div>";
		$content .= "\t\t<a name=\"image\"></a>\n";
		$content .= "\t\t<div id=\"portfolio-image\">\n";
		$content .= "\t\t\t<img id=\"image\" src=\"". $config["path"] ."userfiles/portfolio/" . $image . "\" alt=\"" . $td[0]["portfolio_brief"] . "\" title=\"" . $td[0]["portfolio_title"] . "\" width=\"400\" height=\"500\" />\n";
		$content .= "\t\t</div>\n";
